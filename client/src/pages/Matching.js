...
import { getDocs, deleteDoc, doc, addDoc, collection, updateDoc } from "firebase/firestore";
...
function Matching({ isAuth }) {
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      else if (event.key === 'c') { // Left arrow key
        event.preventDefault();
        handleClose();
  ...
  const handleMatches = async (userId, authorId) => {
    const data = await getDocs(matchesCollectionRef);
    // if the userId already exists as part of the user field in the matches table add the authorId as a new match
    const matches = data.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
    const matchExists = matches.find(
      (match) =>
        match.user.includes(userId) 
    );
    if (matchExists) {
      console.log("updating match")
      // Update the existing match
      const matchDoc = doc(db, "matches", matchExists.id);
      await updateDoc(matchDoc, {
        matchedUsers: [...new Set([...matchExists.matchedUsers, authorId])],
      });
    } else {
  ...
  const seeIfMatch = async () => {
    ...
    if (mutualLikes.length > 0) {
      handleMatches(auth.currentUser?.uid, author);
    }
  ...
  return (
    ...
    <div className="MatchingPage">
        ...
        <div className="m-post" key={currentPost.id}>
          {match && showMatchModal &&
            <div className="popupStyle">
              ...
              <a href ="/the-green-trade-website/matches" className="view">View Matches</a> 
      ...
      <div className="buttons">
        {showMatchModal ?
          <>
            <button className="but" onClick={null}><i className="fa fa-x-dis fa fa-x" aria-hidden="true"></i>
            </button>
            <button className="but" onClick={null}>
              <i className="fa fa-undo-dis fa-undo" aria-hidden="true"></i>
            </button>
            <button className="but" onClick={null}><i className="fa fa-heart-dis fa-heart" aria-hidden="true"></i>
            </button>
          </>
          :
          <>
